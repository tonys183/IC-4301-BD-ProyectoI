    ...
    try {
        const recursiveQuery = `
            WITH RECURSIVE taxon_hierarchy AS (
                SELECT
                    ...
                    CONCAT(t.rank, ': ', t.taxon_name,
                           CASE WHEN t.common_name IS NOT NULL THEN ' (' || t.common_name || ')' ELSE '' END
                    ) AS taxon_line,
                    1 AS level
            ...
            SELECT
                ...
                (
                    SELECT string_agg(th2.taxon_line, ' > ' ORDER BY th2.level DESC)
                    FROM taxon_hierarchy th2
                    WHERE th2.observation_id = o.observation_id
...
function displayObservations(observations) {
    ...
    observations.forEach(obs => {
        ...
        const observationDate = obs.date_obs ? new Date(obs.date_obs).toLocaleDateString() : 'Sin fecha';
